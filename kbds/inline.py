...
def get_payments_btn(*, level: int, sizes: tuple[int] = (2,)):
    keyboard = InlineKeyboardBuilder()
    btns = {
            "Qiwi": "qiwi",
            "Криптовалюта": "crypto",
    ...
    if level == 4:
         keyboard.add(InlineKeyboardButton(text='Назад',
                callback_data=MenuCallBack(level=level - 1, menu_name='order').pack()))
    else:
        keyboard.add(InlineKeyboardButton(text='Назад',
                callback_data=MenuCallBack(level=4, menu_name='order').pack()))
    ...
    for text, menu_name in btns.items():
        ...
        elif menu_name == 'crypto':
            keyboard.add(InlineKeyboardButton(text=text,
                    callback_data=MenuCallBack(level=6, menu_name=menu_name).pack()))
        elif menu_name == 'other':
            keyboard.add(InlineKeyboardButton(text=text,
                    callback_data=MenuCallBack(level=7, menu_name=menu_name).pack()))
